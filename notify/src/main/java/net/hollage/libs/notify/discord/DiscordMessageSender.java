...
/**
 * Discordメッセージ送信実装クラス. [How to Use] String webhookUrl =
 * "https://discord.com/api/webhooks/XXXXX/YYYYY"; MessageSender discordSender = new
 * DiscordMessageSender(webhookUrl); discordSender.sendMessage("Discordへのテストメッセージです");
 */
    ...
    try {
      ...
      if (responseCode != 204) {
        throw new MessageSendRuntimeException(
            "Discord webhook failed. HTTP error code: " + responseCode);
      }
    } catch (IOException e) {
